FactoryBot.define do
  factory :event do
#    Event(start: date, finish: date, leavetype_id: integer, absence: boolean, registration_id: integer, daystarted: integer, dayfinished: integer, annotation_id: integer, residual: boolean, confirmed: boolean, archived: boolean, supportingdocumentation: string)
...
#     start "#{latest_payroll.start+offset.days}"
  ...
  #   leavetype_id 1
      sequence(:registration_id) {|n| "#{reg_ids_active_today[n]}" }
   #
