...
# 	[:streetname_id, :municipality_id, :country_id].each do |ids|
	...
	# Address validation - skipped if an internal address such as a classroom on the same building
	...
	validates :postalcode, presence: true, length:  { minimum: 5, maximum: 15  }, unless: :skip_validation
...
# 	scope :with_institution, lambda { joins(:institution) }
	...
	def street
	  	   ...
	  	   [self.streetname.designation, addr, streetnum, complement].join(" ")

  		 else

				 if streetname_id.present?

		  	   [self.streetname.designation, addr, streetnum].join(" ")
	...
	def street_name
  	   [self.streetname.designation, addr].join(" ")
	...
	def mailing
		...
		street_name=[self.streetname.designation, self.addr].join(" ")
		street_addr=street_name+', '+self.streetnum.to_s+' - '+self.complement
		neighborhood=self.neighborhood

		city_and_state=self.municipality.name+' ('+self.municipality.stateregion.state.abbreviation+')'
		# Alternatively: use namewithstate (denormalized data) once this is properly implemented on municipalities controller

		postal_code=I18n.t('activerecord.attributes.address.postalcode')+' '+self.postalcode
	...
	def correspondence
		...
		country=self.municipality.stateregion.state.country.name

		return [self.mailing,country].join(" - ")
	...
	def state_abbreviation
		...
		self.municipality.stateregion.state.abbreviation
