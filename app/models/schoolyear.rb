...
# Into which a student is registered (Reviewed on February 2020)
class Schoolyear < ActiveRecord::Base
  ...
  scope :ordered_by_programname_and_year, -> {
                                            joins(program: :programname)
                                              .reorder('programnames.name,schoolyears.programyear')
                                          }

  scope :pap, -> { joins(:program).merge(Program.pap) }

  scope :medres, -> { joins(:program).merge(Program.medres) }
  ...
  # More specific, given the program's duration
  validate :valid_program_year # , on: :update

  # 366 days times 24 hours / day = 8784 hours maximum (leap year case)
  ...
  def self.for_users_institution(user)
    from_users_institution(user)
  end

  # New formulation
  def self.from_users_institution(user)
    joins(:program).merge(Program.from_users_institution(user))
  end
  ...
  # query logic is similar to self.ids_contextual_on(specified_dt)
  def self.ids_start_on(specified_dt)
    query = "with Intervals_CTE AS (select s.id, ((s.programyear-1)::VARCHAR || ' year')"\
   '::interval as intervl, s.program_id, s.programyear,t.start, t.finish from schoolyears s, '\
   'programs p, schoolterms t where s.program_id=p.id and p.schoolterm_id=t.id) '\
   ', '\
   ...
   'select id from Schoolyear_CTE where start=?'
    ...
    find_by_sql [query, specified_dt]
  end
  ...
  def self.start_on(specified_dt)
    where(id: Schoolyear.ids_start_on(specified_dt))
  end
  ...
  delegate :schoolyears, to: :program, prefix: true # All schoolyears which belong to the program
  ...
  def start_year_i18n
    I18n.t('start') + ' ' + start.year.to_s
  end

  def start
    seniority = (programyear - 1).year # > 0 when program duration > 1
    school_term.start + seniority
  end
